...
def Profile():
    ...
    results = db.execute("SELECT date, score, advice, recipe FROM results JOIN users on userID = id WHERE userID = ? ORDER BY date", session["userID"])
    Name = db.execute("SELECT DISTINCT name FROM users WHERE id = ?", session["userID"])
    name = Name[0]["name"]
...
def Results():
    ...
    if score <= 9:
        stars = "★☆☆"
        title = "Dietary Disaster"
        comment = "Needs improvement."
    elif score > 9 and score <=18:
        stars = "★★☆"
        ...
        stars = "★★★"
        title = "Health Hero 🏆"
        comment = "Amazing! You follow most of the dietary advice!"

    # Put into results database
    db.execute("INSERT INTO results (userID, date, score, advice, recipe) VALUES(?, ?, ?, ?, ?)", session["userID"], date, score, advice, recipe)
    return render_template("Results.html", score = score, advice = advice, recipe = recipe, stars = stars, title = title, comment = comment)
